...
setup :: proc(p: ^probe.Probe, name: string) {
	p.root = fmt.aprintf("%s/%s", p.base, name)
	probe.source(p, "odx.json5", CONFIG)
	probe.source(p, "app/app.odin", "package app\nimport \"local:dep\"\n")
	probe.source(p, "dep/dep.odin", "package dep\nvalue :: 42\n")
}
...
run :: proc(p: ^probe.Probe, name: string, code: int, args: []string = nil) -> probe.Report {
	cmd := make([dynamic]string)
	append(&cmd, "check", "--fast", "--topic", "dependencies", "--json")
	append(&cmd, ..args)
	out := probe.run(p, name, code, cmd[:])
	r: probe.Report
	jerr := json.unmarshal_string(out, &r)
	probe.expect(
		p,
		jerr == nil && r.coverage.source_scope != "",
		fmt.tprintf("%s: coverage JSON", name),
	)
	return r
}
findings :: proc(r: probe.Report, pkg := "app/") -> string {
...
architecture_complete :: proc(r: probe.Report, pkg := "app") -> bool {
...
test_architecture :: proc(t: ^testing.T) {
	...
	probe.expect(&p, findings(dir) == findings(repeat), "dependency witness deterministic")

	setup(&p, "unassigned")
	probe.source(&p, "app/app.odin", "package app\nimport \"local:unassigned\"\n")
	probe.source(&p, "unassigned/u.odin", "package unassigned\nimport \"core:os\"\n")
	...
	for target in ([]string{"excluded", "missing"}) {
		setup(&p, target)
		probe.source(&p, "app/app.odin", fmt.tprintf("package app\nimport \"local:%s\"\n", target))
		if target ==
		   "excluded" {probe.source(&p, "excluded/e.odin", "package excluded\nimport \"core:os\"\n")}
	...
	probe.source(&p, "odx.json5", unknown_config)
	probe.source(&p, "app/app.odin", "package app\nimport \"unknown:thing\"\n")
	r = run(&p, "unknown collection", 2, {"app"})
	probe.expect(&p, !architecture_complete(r), "unknown collection is not a clean leaf")
	...
	setup(&p, "tests")
	probe.source(&p, "dep/dep_test.odin", "package dep\nimport \"core:os\"\n")
	...
	probe.source(&p, "app/app_test.odin", "package app\nimport \"core:os\"\n")
	r = run(&p, "selected package test imports", 1, {"app"})
	probe.expect(&p, findings(r) != "", "selected package test imports checked")

	setup(&p, "platform")
	platform := "windows" if ODIN_OS != .Windows else "linux"
	probe.source(&p, fmt.tprintf("dep/inactive_%s.odin", platform), "package dep\nimport \"core:os\"\n")
	r = run(&p, "inactive platform source", 1, {"app"})
	probe.expect(&p, findings(r) != "", "inactive platform edges retained")

	setup(&p, "malformed")
	probe.source(&p, "unrelated/broken.odin", "package unrelated\nbroken :: proc(\n")
	r = run(&p, "unrelated malformed source", 0, {"app"})
	probe.expect(&p, architecture_complete(r), "unrelated parse failure does not poison reachability")
	probe.source(&p, "dep/dep.odin", "package dep\nbroken :: proc(\n")
	r = run(&p, "reached malformed source", 2, {"app"})
	probe.expect(&p, !architecture_complete(r), "reached parse failure prevents compliance")
	...
	setup(&p, "diamond")
	probe.source(&p, "dep/dep.odin", "package dep\nimport \"local:left\"\nimport \"local:right\"\n")
	probe.source(&p, "left/left.odin", "package left\nimport \"local:bottom\"\n")
	probe.source(&p, "right/right.odin", "package right\nimport \"local:bottom\"\n")
	probe.source(&p, "bottom/bottom.odin", "package bottom\nimport \"core:os\"\n")
	...
	probe.expect(&p, !architecture_complete(r), "builtin collection escape evidence unavailable")
	...
	probe.source(&p, "odx.json5", shadow_config)
	probe.source(&p, "app/app.odin", "package app\nimport \"core:dep\"\n")
	...
	probe.source(&p, "odx.json5", outside_config)
	probe.write(fmt.tprintf("%s/external/dep/dep.odin", p.base), "package dep\nvalue :: 42\n")
	r = run(&p, "configured external collection", 2, {"app"})
	probe.expect(&p, !architecture_complete(r), "configured external collection evidence unavailable")
	...
	probe.source(&p, "odx.json5", changed_config)
	r = run(&p, "config-only change", 0, {"--since", "HEAD"})
	probe.expect(&p, len(r.coverage.packages) == 2, "config-only change scans project")
	probe.source(&p, "odx.json5", CONFIG)
	probe.source(&p, "dep/other.odin", "package dep\nvalue :: 42\n")
	commit(&p)
	...
	probe.expect(&p, len(r.coverage.packages) == 2, "deleted source scans surviving project")
	commit(&p)
	...
	probe.expect(&p, len(r.coverage.packages) == 2, "renamed source scans project")
	commit(&p)
	probe.source(&p, ".odx/topics/research-note.txt", "Project policy change discovery probe.\n")
	r = run(&p, "local policy-only change", 0, {"--since", "HEAD"})
	probe.expect(&p, len(r.coverage.packages) == 2, "local policy-only change scans project")

	setup(&p, "performance")
	for i in 0 ..< 100 {probe.source(&p, fmt.tprintf("pkg%03d/p.odin", i), fmt.tprintf("package pkg%03d\nvalue :: %d\n", i, i))}
