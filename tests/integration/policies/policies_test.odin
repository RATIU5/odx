...
check :: proc(p: ^probe.Probe, name: string, expected: []string, scope := "") -> probe.Report {
	args := make([dynamic]string)
	append(&args, "check", "--strict", "--json")
	if scope != "" {append(&args, scope)}
	output := probe.run(p, name, 1 if len(expected) > 0 else 0, args[:])
	r: probe.Report
	probe.expect(p, json.unmarshal_string(output, &r) == nil, fmt.tprintf("%s report decodes", name))
	probe.expect(p, r.coverage.complete, fmt.tprintf("%s evidence complete", name))
	probe.expect(p, len(r.violations) == len(expected), fmt.tprintf("%s exact finding count", name))
	for id in expected {
		found := false
		for v in r.violations {if v.rule == id {found = true}}
		probe.expect(p, found, fmt.tprintf("%s reports %s", name, id))
	}
	return r
}
copy_example :: proc(p: ^probe.Probe, name: string) {
...
guidance :: proc(p: ^probe.Probe, ids: []string) {
	probe.run(p, "write project guidance", 0, {"policy", "--write", "AGENTS.md"})
	probe.run(p, "project guidance fresh", 0, {"policy", "--verify", "AGENTS.md"})
	text := probe.read(p, "AGENTS.md")
	for id in ids {probe.expect(p, strings.contains(text, fmt.tprintf("**%s**", id)), fmt.tprintf("guidance includes %s", id))}
	probe.expect(
		p,
		strings.count(text, "\n- **") == len(ids),
...
cases :: proc(p: ^probe.Probe, target: string, variants: []Case, replace: bool) {
	original := ""
	if replace {original = probe.read(p, target)}
	for c in variants {
		probe.source(p, target, probe.read(p, fmt.tprintf("cases/%s.odin.txt", c.name)))
		expected := []string{}
		if c.rule != "" {
	...
	if replace {probe.source(p, target, original)} else {
		if err := os.remove(fmt.tprintf("%s/%s", p.root, target));
		   err != nil {panic(fmt.tprintf("%v", err))}
	}
}
audit_config :: proc(p: ^probe.Probe, value: string, extra := "") {
	probe.source(
		p,
		"odx.json5",
		strings.concatenate(
...
test_independent_policy_projects :: proc(t: ^testing.T) {
	...
	for id in ([]string{"odx/feature-optout", "odx/vet-disable"}) {
		...
		probe.expect(&p, found, fmt.tprintf("disabled audit %s is not applicable", id))
	}
	guidance(&p, {"library/R1", "library/R2"})
	minimal_md := probe.read(&p, "AGENTS.md")
	...
	config := probe.read(&p, "odx.json5")
	disabled_config, _ := strings.replace_all(
		config,
		`"errors/R3":`,
		`"library/R1":"Temporarily disabled", "errors/R3":`,
	)
	probe.source(&p, "odx.json5", disabled_config)
	probe.source(&p, "parser/main.odin", probe.read(&p, "cases/mutable.odin.txt"))
	check(&p, "disabled local rule is inactive in normal checks", {})

	probe.source(&p, "odx.json5", config)
	probe.source(&p, "parser/main.odin", probe.read(&p, "cases/misleading_text.odin.txt"))
	probe.source(
		&p,
		"support/main.odin",
	...
	if len(boundary.violations) ==
	   1 {probe.expect(&p, boundary.violations[0].file == "support/main.odin", "foreign report belongs to declaration package")}
	if err = os.remove_all(fmt.tprintf("%s/support", p.root));
	   err != nil {panic(fmt.tprintf("%v", err))}
	probe.source(&p, "parser/main.odin", probe.read(&p, "cases/misleading_text.odin.txt"))
	path := ".odx/topics/library/R1.odx.md"
	original := probe.read(&p, path)
	probe.source(
		&p,
		path,
	...
	probe.run(&p, "local rule mutation stales guidance", 1, {"policy", "--verify", "AGENTS.md"})
	probe.expect(&p, probe.read(&p, "AGENTS.md") == before, "freshness check does not mutate guidance")
	probe.run(&p, "regenerate changed local guidance", 0, {"policy", "--write", "AGENTS.md"})
	probe.run(&p, "changed local guidance fresh", 0, {"policy", "--verify", "AGENTS.md"})
	probe.source(&p, path, original)
	...
	probe.source(&p, "domain/case.odin", probe.read(&p, "cases/transitive.odin.txt"))
	...
	if err = os.remove(fmt.tprintf("%s/domain/case.odin", p.root));
	   err != nil {panic(fmt.tprintf("%v", err))}
	probe.run(&p, "app scoped guidance", 0, {"policy", "--write", "APP.md", "app"})
	probe.run(&p, "app scoped guidance fresh", 0, {"policy", "--verify", "APP.md", "app"})
	app := probe.read(&p, "APP.md")
	probe.expect(&p, strings.count(app, "\n- **") == 0, "app guidance has no domain mechanical rules")
	...
	probe.source(&p, "sample/main.odin", "#+feature using-stmt\n#+vet !tabs\npackage sample\n")
	audit_config(&p, "audit_file_tags:false,")
	check(&p, "audit opt out", {})
	probe.run(&p, "audit off guidance", 0, {"policy", "--write", "AGENTS.md"})
	audit_config(&p, "")
	probe.run(&p, "audit setting change stales guidance", 1, {"policy", "--verify", "AGENTS.md"})
	...
	for value in ([]string{"null", "0", `"false"`}) {
		...
		probe.run(&p, "invalid audit boolean rejected", 2, {"check", "--fast"})
	}
